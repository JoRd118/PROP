  ...
  public SolucioQAP Calcular(int indexPlanetaCalcul, String[] mPlanetes, String[] mRecursos, 
    ...
    if(necessitatsPendents.size() < 1) {
      ...
      if(hiHaSolucio && (planetesPendents.size() == 1)) {
        ...
        int idAltrePla = planetesPendents.get(0);
        double distancia = mDistancies[indexPlanetaCalcul][idAltrePla];
        sol.modificarTerraformat(true);
        sol.modificarMillorRecorregut(distancia);
        sol.modificarObservacions("Nomes hi ha un planeta possible: "+mPlanetes[idAltrePla]);
        // Node Arrel
        ArrayList<Integer> camiPla = new ArrayList<Integer>();
        camiPla.add(indexPlanetaCalcul);
        ArbreNode mutArrel = new ArbreNode(mPlanetes[indexPlanetaCalcul],0,0);
        bbNode nodeSel = new bbNode(0, -1, indexPlanetaCalcul, false, false, 0, 0, 0, 0, 0, 
                                    camiPla, necessitatsPendents, planetesPendents,mutArrel);
        ArrayList<bbNode> llistaNodes = new ArrayList<bbNode>();
        llistaNodes.add(nodeSel);
        // Segon node
        camiPla.add(idAltrePla);
        ArbreNode mutNode = new ArbreNode(mPlanetes[idAltrePla],1,1);
        nodeSel = new bbNode(1, 0, idAltrePla, true, false, 1, distancia, 0, 0, 0, 
                                   camiPla, null, null,mutNode);
        mutArrel.afegirFill(mutNode);
        llistaNodes.add(nodeSel);
